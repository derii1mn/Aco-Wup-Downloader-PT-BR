...
# Patched version with support for more keysites and updates blocked
...
class RootWindow(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.versions = {'gui_new': '', 'gui_all': '', 'gui_url': 'ALL YOUR UPDATES ARE BELONG TO US !!'
                         #, 'gui_url': 'https://github.com/GeovanniMCh/ACO_WUP_DOWNLOADER/releases'
                         }
        self.runningversion = __VERSION__
        #self.targetversion = "ACOWD mod"
        self.targetversion = "Aco Wup Downloader y Patch"
    ...
    def notify_of_update(self, update=True):
        if update:
            ...
            fg = None
            self.updatelabel.configure(text="PATCHED", foreground=fg, font="Helvetica 13 bold")
    ...
    def load_program_revisions(self):
        # Actual update checker, also removed
...
if __name__ == '__main__':
    root = RootWindow()
    root.title('ACO WUP DOWNLOADER - Patched')
               # 3.0.0')
